...
local function config_path(multiinstance)
	...
	if ( symlink and source:match('^/etc/') ) or source:match('^/usr/') then
		-- system wide. either /etc/{instance}/conf.lua or /etc/{appname}/{instance}.lua
		if multiinstance then
			-- /etc/{appname}/{instance}.lua
...
addpaths('path', 'app/?.lua', 'app/?/init.lua', 'libs/share/lua/5.1/?.lua', 'libs/share/lua/5.1/?/init.lua')
